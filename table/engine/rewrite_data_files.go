...
func RewriteDataFiles(ctx context.Context, t *Transaction, groups []CompactionTaskGroup, partialProgress bool, snapshotProps iceberg.Properties) (*RewriteResult, error) {
	if len(groups) == 0 {
	...
	for _, group := range groups {
		...
		for df, err := range WriteRecords(ctx, t.Table(), arrowSchema, records) {
			if err != nil {
	...
	if len(allOldData) > 0 {
		...
		t.AppendValidator(RewriteValidatorFor(rewritten))
	}
