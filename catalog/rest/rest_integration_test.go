...
//	s.Require().NoError(err)
//
//	_, err = s.cat.CreateView(s.ctx, catalog.ToIdentifier(TestNamespaceIdent, "test-view"), viewVersion, tableSchemaSimple, iceberg.Properties{"foobar": "baz"})
//	s.Require().NoError(err)
//
...
//	createdView, err := s.cat.CreateView(s.ctx, viewIdent, viewVersion, tableSchemaSimple, testProps)
//	s.Require().NoError(err)
//
...
//	s.Equal(location, updatedView.MetadataLocation())
//	s.Equal(updatedMD, updatedView.Metadata())
